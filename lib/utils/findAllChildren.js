...
function findAll(children, searchObj, childrenKey) {
  ...
  var _loop = function _loop(i) {
    ...
    if (children[i][childrenKey] && children[i][childrenKey].length > 0) {
      var newLocations = findAll(children[i][childrenKey], searchObj, childrenKey, [].concat(_toConsumableArray(parents), [i]), locations);
